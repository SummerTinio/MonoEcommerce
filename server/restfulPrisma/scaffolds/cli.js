...
const usageMsg =
  'Generate REST Controller and Repository.\nUsage: make:rest <Entity> [<table>]';
...
function addRoute(entityName) {
  ...
  data.splice(
    data.indexOf('module.exports = [\r') + 1,
    0,
...
function copyEntity(filePath, name, type, callback) {
  fs.copyFile(
    `${templatePath}/Entity${type}.ts`,
    `${filePath}/${name}${type}.ts`,
    COPYFILE_EXCL,
    callback,
  );
}
...
if (args[0] == 'make:rest') {
  ...
  copyEntity(controllerPath, EntityName, 'Controller', () =>
    replaceEntity(`${controllerPath}/${EntityName}Controller.ts`, EntityName),
  );
  copyEntity(repositoryPath, EntityName, 'Repository', () =>
    replaceEntity(`${repositoryPath}/${EntityName}Repository.ts`, EntityName),
  );
  addRoute(EntityName);
