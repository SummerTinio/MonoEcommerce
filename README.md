...
- Considerations on consistency between browsers
  - made use of widely-supported CSS features (caniuse.com)
  - used a minimal, but complete CSS reset to ensure baseline for all clients/browsers
    - minimal - to not pollute styles unnecessarily
    - complete - to ensure all html elements start from the same baseline
  - used styled components, which has built-in vendor-prefixing upon compilation
    - automatically adds --webkit- prefixes
- Considerations for accessibility
  - used NextJS for static site generation - better for screenreaders, more aria polite
  - used semantic markup wherever possible (to convey meaning through html tags/elements)
